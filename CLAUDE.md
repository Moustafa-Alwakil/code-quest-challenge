...
This section overrides the Boost guidelines above wherever the two disagree.

## Enum keys are SCREAMING_SNAKE_CASE

`PLATFORM_CASH`, `PERIOD_RECOGNIZED`, `ACTIVE` — not `PlatformCash`. Enum cases are constants, and
the case says so at the call site. Backing string values stay `snake_case` (`'platform_cash'`), so
the spelling change never touches stored data.

Boost's `php rules` block says TitleCase. It is regenerated by `boost:install`, so it will keep
saying that — this section wins. Recorded as R24 in `docs/features/README.md`; do not "fix" an
enum back to TitleCase on the strength of the generated block alone.
