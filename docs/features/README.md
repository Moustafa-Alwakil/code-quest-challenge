...
### Seeder refinements (F02 completion)

Recorded when F02's seeders were built. Both pin the boundary of the bulk write path, which is the
one place in the system that trades a correctness guarantee for throughput.

| # | Refinement | Where | Why |
|---|---|---|---|
| R41 | `LedgerService::postMany()` writes many transactions in one `insertOrIgnore`, and **applies no balance deltas**. It is for postings that move no instructor's snapshot — at scale, the `payment_received` entries of fifty thousand terms. A batch that needed deltas would need them merged across transactions and ordered by instructor id, which is `post()`'s job, one posting at a time | F02, F03 | D-9 says the ledger is written by `LedgerService` and nothing else, and this keeps that true rather than letting a seeder reach for `DB::table('ledger_entries')`. Each transaction still gets its own `transaction_uuid`, because "this posting balances" (I2) is a statement about one business fact and merging them would make it unprovable. The no-deltas restriction is the load-bearing half: R18 put "deltas apply *only if* the legs were inserted" in exactly one place, and a batch variant that quietly skipped them would be a second place where that rule does not hold |
| R42 | `SubscribeStudentsInBulkAction` is **not** an entry point. It uses `insert()` rather than `insertOrIgnore()`, derives subscription and payment ids from MySQL's consecutive-autoincrement guarantee (a single multi-row `INSERT` reports its first id; under the default `innodb_autoinc_lock_mode = 1` the rest follow in row order), and assumes its caller has checked the amount against the plan. `ScaleSeeder` therefore asserts an empty `subscriptions` table instead of pretending to be idempotent | F02, F04 | A payment arriving from a gateway still goes through `SubscribeStudentAction`, where the replay check and the UNIQUE `external_ref` race are what make it safe. Fifty thousand terms through that path is fifty thousand transactions and a seeder nobody waits for — but the trade has to be visible, because the id derivation breaks silently the moment one row is skipped, and the failure would surface thousands of rows later as an integrity error nobody could place. Refusing to run on a non-empty table is the honest form of that: a loud precondition rather than a quiet corruption. Sizing lives in `config/scale.php` rather than `config/revenue.php`, since nothing about what a piastre is worth changes with it |
